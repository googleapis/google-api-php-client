...
/**
 * Abstracts the methods of the discovered Google API.
 *
 * @author Silvano Luciani <silvano@google.com>
 *
 */
class Google_Service_Method
{
  // Method name
  private $methodName;
  ...
  public function __construct($methodName, $service, $client) {
   ...
   */
  public function execute($arguments, $expected_class = null)
  {
    $method = $this->methods[$name];
    $parameters = $arguments[0];

    // postBody is a special case since it's not defined in the discovery d
...
//      }
    ...
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) &&
            $paramSpec['required'] && ! isset($parameters[$paramName])) {
    ...
    $contentType = false;
    // TODO(ianbarber): Should probably check for compatible mime types
    // with the media upload flag.
    ...
    $data = Google_Http_REST::execute($this->client, $httpRequest);
    if ($expected_class) {
      $data = new $expected_class($data);
    }

    return $data;
  }

  // TODO(ianbarber): Is this a legit pass-by-reference? I actually
  // have no clue what this is doing this for.
