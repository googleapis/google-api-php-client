...
class Google_Service_YouTube_LiveBroadcasts_Resource extends Google_Service_Resource
{
   ...
   * The displaySlate parameter specifies whether to enable or disable the slate.
   * @opt_param string offsetTimeMs
   * The offsetTimeMs parameter specifies a point in time in the video when the specified action
    * (e.g. display a slate) is executed. The property value identifies a positive time offset, in
    * milliseconds, from the beginning of the monitor stream. Though measured in milliseconds, the
    * value is actually an approximation, and YouTube will act as closely as possible to that time. If
    * not specified, it indicates that the action should be performed as soon as possible. If your
    * broadcast stream is not delayed, then it should not be specified. However, if your broadcast
    * stream is delayed, then the parameter can specify the time when the operation should be
    * executed. See the Getting started guide for more details. Note: The offset is measured from the
  ...
  public function listLiveBroadcasts($part, $optParams = array())
  {
    $params = array('part' => $part);
    $params = array_merge($params, $optParams);
    return $this->call('list', array($params), "Google_Service_YouTube_LiveBroadcastList");
  }
  /**
   ...
   * The broadcastStatus parameter identifies the state to which the broadcast is changing.
   * @param string $id
...
class Google_Service_YouTube_LiveStreams_Resource extends Google_Service_Resource
{
  ...
  public function listLiveStreams($part, $optParams = array())
  {
    $params = array('part' => $part);
    $params = array_merge($params, $optParams);
    return $this->call('list', array($params), "Google_Service_YouTube_LiveStreamList");
  }
  /**
...
class Google_Service_YouTube_PlaylistItems_Resource extends Google_Service_Resource
{
  ...
  If the parameter identifies a property
    ...
    * example, in a playlistItem resource, the snippet property contains numerous fields, including
    * the title, description, position, and resourceId properties. As such, if you set part=snippet,
...
class Google_Service_YouTube_Search_Resource extends Google_Service_Resource
{
   ...
   * The forContentOwner parameter restricts the search to only retrieve resources owned by the
    * content owner specified by the onBehalfOfContentOwner parameter. The user must be authenticated
    * as a CMS account linked to the specified content owner and onBehalfOfContentOwner must be
    * provided.
...
class Google_Service_YouTube_Thumbnails_Resource extends Google_Service_Resource
{
  ...
  public function set($videoId, $optParams = array())
  {
    ...
    $params = array_merge($params, $optParams);
    return $this->call('set', array($params), "Google_Service_YouTube_ThumbnailListResponse");
  }
}
...
class Google_Service_YouTube_Videos_Resource extends Google_Service_Resource
{
   ...
   * The part parameter serves two purposes in this operation. It identifies the properties that the
    * write operation will set as well as the properties that the API response will include.
  The part
    * names that you can include in the parameter value are snippet, contentDetails, player,
    * statistics, status, and topicDetails. However, not all of those parts contain properties that
    * can be set when setting or updating a video's metadata. For example, the statistics object
    * encapsulates statistics that YouTube calculates for a video and does not contain values that you
    * can set or modify. If the parameter value specifies a part that does not contain mutable values,
  ...
  The onBehalfOfContentOwnerChannel parameter
    * indicates that the request's authorization credentials identify a YouTube CMS user who is acting
    * on behalf of the channel specified in the parameter value. This parameter must be used in
    * conjunction with the onBehalfOfContentOwner parameter, and the user must be authenticated using
    * a CMS account that is linked to the content owner that the onBehalfOfContentOwner parameter
    * specifies. In addition, the channel that the onBehalfOfContentOwnerChannel parameter value
   ...
   * Set this parameter's value to mostPopular to instruct the API to return videos belonging to the
    * chart of most popular videos.
   * @opt_param string id
   * The id parameter specifies a comma-separated list of the YouTube video ID(s) for the resource(s)
    * that are being retrieved. In a video resource, the id property specifies the video's ID.
   * @opt_param string locale
   * The locale parameter selects a video chart available in the specified locale. If using this
    * parameter, chart must also be set. The parameter value is an BCP 47 locale. Supported locales
    * include ar_AE, ar_DZ, ar_EG, ar_JO, ar_MA, ar_SA, ar_TN, ar_YE, cs_CZ, de_DE, el_GR, en_AU,
    * en_BE, en_CA, en_GB, en_GH, en_IE, en_IL, en_IN, en_KE, en_NG, en_NZ, en_SG, en_UG, en_US,
    * en_ZA, es_AR, es_CL, es_CO, es_ES, es_MX, es_PE, fil_PH, fr_FR, hu_HU, id_ID, it_IT, ja_JP,
    * ko_KR, ms_MY, nl_NL, pl_PL, pt_BR, ru_RU, sv_SE, tr_TR, zh_HK, zh_TW
   * @opt_param string maxResults
   * USE_DESCRIPTION --- channels:list:maxResults
   ...
   * USE_DESCRIPTION --- channels:list:pageToken
   * @opt_param string videoCategoryId
   * The videoCategoryId parameter selects a video chart based on the category. If using this
    * parameter, chart must also be set.
  ...
  Note that this method will override the existing values
    * for all of the mutable properties that are contained in any parts that the parameter value
    * specifies. For example, a video's privacy setting is contained in the status part. As such, if
    * your request is updating a private video, and the request's part parameter value includes the
    * status part, the video's privacy setting will be updated to whatever value the request body
    * specifies. If the request body does not specify a value, the existing privacy setting will be
    * removed and the video will revert to the default privacy setting.
  In addition, not all of those
    * parts contain properties that can be set when setting or updating a video's metadata. For
    * example, the statistics object encapsulates statistics that YouTube calculates for a video and
...
class Google_Service_YouTube_ActivityContentDetailsPromotedItem extends Google_Model
{
  public $adTag;
...
class Google_Service_YouTube_LiveBroadcastList extends Google_Collection
{
  public $etag;
  public $eventId;
...
{
  protected $cdnType = 'Google_Service_YouTube_LiveStreamCdn';
...
class Google_Service_YouTube_LiveStreamCdn extends Google_Model
{
  public $format;
...
class Google_Service_YouTube_LiveStreamList extends Google_Collection
{
  public $etag;
  public $eventId;
...
class Google_Service_YouTube_ThumbnailListResponse extends Google_Collection
{
  public $etag;
